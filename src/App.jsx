...
function App() {
  ...
  const handleBallMargin = () => {
   ...
   } // <-- Ball Click Logic: If ball margin equals 0 pixel, the web is in daytime mode, and if it is not equal, it is in nighttime mode.

  const getData = async () => {
    try {
      const response = await axios.get('https://api.dictionaryapi.dev/api/v2/entries/en/' + inputValue)
      // Filter Phonetic Functions
      const filterPhonetic = response.data[0].phonetics.filter(phonetic => {
        return phonetic.hasOwnProperty('text') === true && phonetic.hasOwnProperty('audio') === true
      })
      const filterTextAudio = filterPhonetic.filter(phonetic => {
        return phonetic.text !== '' && phonetic.audio !== ''
      })
      setPhonetic(filterTextAudio[0].text)
      setAudioSrc(filterTextAudio[0].audio)
      console.log(filterTextAudio)


      // const phoneticss = response.data[0].phonetics.filter(phon => {
      //     return phon.audio !== '';
      // })

      // const result = phoneticss[0]
  ...
  return (
    <div className="App" style={{backgroundColor: ballMargin == '0px'? '#FFF': '#050505'}}>
      ...
      <svg className='book-svg' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 30 34" fill="none">
        ...
        <path d="M9.76709 8.0138H20.2876" stroke="#757575" strokeWidth="1.5" strokeLinecap="round"/>
      </svg>
        ...
        <div className="right-div">
          ...
          <div className="font-family-div">
            ...
            {/* Font Family List  */}
            {fontFamilyList &&
             <div className="font-family-list" style={{backgroundColor: ballMargin == '0px'? '#FFF': '#1F1F1F',
            color: ballMargin == '0px'? '#2D2D2D': '#FFFFFF',
            boxShadow: ballMargin == '0px'? '0px 5px 30px 0px rgba(0, 0, 0, 0.10)': ' 0px 5px 30px 0px #A445ED'}}>

              <p onClick={() => {setFontFamily('sans-serif'); setFontFamilyList(!fontFamilyList)}}
               style={{fontFamily: 'sans-serif'}} className='sans-serif'>Sans Serif</p>
              <p onClick={() => {setFontFamily('serif'); setFontFamilyList(!fontFamilyList)}}
               style={{fontFamily: 'serif'}} className='serif'>Serif</p>
              <p onClick={() => {setFontFamily('monospace'); setFontFamilyList(!fontFamilyList)}}
               style={{fontFamily: 'monospace'}} className='mono'>Monospace</p>
          ...
          <div className='day-night-div'>
            <div className="day-night-changer" style={{backgroundColor: ballMargin == '0px'? '#757575': '#A445ED'}}>
              <div style={{marginLeft: ballMargin}} onClick={() => handleBallMargin()}></div>
            </div>
            ...
            <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 22 22" fill="none">
              <path fillRule="evenodd" clipRule="evenodd" d="M1 10.449C0.998458 12.8283 1.80169 15.1383 3.27914 17.0033C4.75659 18.8683 6.82139 20.1788 9.13799 20.7218C11.4545 21.2647 13.8866 21.0082 16.039 19.994C18.1912 18.9797 19.9373 17.2673 20.9931 15.1352C11.5442 15.1352 6.85799 10.4479 6.85799 1C5.09842 1.87311 3.61767 3.22033 2.58266 4.88981C1.54765 6.5593 0.999502 8.48469 1 10.449Z"
               style={{stroke: ballMargin == '0px'? '#757575': '#A445ED'}} strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </div>
        </div>
      </div>
      ...
      <div className="input-parent-div">
        <div className="input-div" style={{backgroundColor: ballMargin == '0px'? '#F4F4F4': '#1F1F1F',
        ...
        {/* Search Button */}
        <div onClick={() => getData()} className='search-div'>
      ...
      {inputValue == ''? <p className='warning' style={{fontFamily: fontFamily}}>Whoops, can’t be empty…</p>: null}
      </div>
      ...
      <div className="result">
        <div className="word-result">
          ...
          <div className="word-pronounce">
            <p style={{fontFamily: fontFamily, color: ballMargin == '0px'? '#2D2D2D': '#FFF'}} className="word">{inputValue}</p>
            <p className="word-saying-form">{phonetic}</p>
          </div>

          {/* Word Audio  */}
          <svg onClick={() => new Audio(audioSrc).play()} className='audio-player' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 48 48" fill="none">
            <circle opacity='0.25' cx="24" cy="24" r="24" fill="#A445ED"/>
            <path fillRule="evenodd" clipRule="evenodd" d="M19 18V31L32 24.5L19 18Z" fill="#A445ED"/>
          </svg>
        </div>

        {/* Noun  */}
        <div className="noun-verb">
          <p style={{fontFamily: fontFamily, color: ballMargin == '0px'? '#2D2D2D': '#FFF'}}>noun</p>
          <hr style={{backgroundColor: ballMargin == '0px'? '#E9E9E9': '#3A3A3A'}}/>
        </div>

        {/* Meaning  List */}
        {/* Noun Meaning  */}
        <p style={{fontFamily: fontFamily}} className='meaning'>Meaning</p>
        <div className='list-div'>
          <div className="list-item">
            <li></li>
            <p style={{fontFamily: fontFamily, color: ballMargin == '0px'? '#2D2D2D': '#FFF'}}>
              {}
              </p>
          </div>
        ...
        <div className="synonyms-div">
          <p style={{fontFamily: fontFamily}} className='synonym-title'>Synonyms</p>
          <p className="synonym" style={{fontFamily: fontFamily}}>{}</p>
        </div>

        {/* Verb  */}
        <div className="noun-verb">
          <p style={{fontFamily: fontFamily, color: ballMargin == '0px'? '#2D2D2D': '#FFF'}}>Verb</p>
          <hr style={{backgroundColor: ballMargin == '0px'? '#E9E9E9': '#3A3A3A'}}/>
        </div>
        ...
        <div className='list-div'>
          ...
          {/* Footer Border */}
          <hr className='footer-border' style={{backgroundColor: ballMargin == '0px'? '#E9E9E9': '#3A3A3A'}}/>

          {/* Source  */}
          <p className='source' style={{fontFamily: fontFamily}}>Source</p>
          <div className="source-div">
            <a style={{fontFamily: fontFamily, color: ballMargin == '0px'? '#2D2D2D': '#FFF'}} target='_blank'
             href={{}}>{}</a>
